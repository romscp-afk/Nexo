#!/usr/bin/env node
/**
 * Deploy submit-contact edge function + email secrets.
 * Usage: npm run deploy:submit-contact
 */
...
const functionSlug = 'submit-contact'
...
console.log('Deploying submit-contact…')
...
const secrets = [
  {
    name: 'SITE_URL',
    value: fileEnv.SITE_URL ?? process.env.SITE_URL ?? 'https://nexoservice.online',
  },
]
...
if (!resendKey) {
  console.log('')
  console.log('Note: RESEND_API_KEY not in .env — admin emails will skip until configured.')
}
