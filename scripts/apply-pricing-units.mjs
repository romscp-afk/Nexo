#!/usr/bin/env node
/**
 * Apply supabase/add-pricing-units.sql (unit pricing, platform fee SGD 3).
 * Usage: npm run setup:pricing
 */
...
const sql = readFileSync(join(root, '..', 'supabase', 'add-pricing-units.sql'), 'utf8')

async function applyViaManagementApi() {
  const fileEnv = loadEnv()
  const token = requireAccessToken(fileEnv)
  console.log('Applying pricing migration via Supabase Management API…')
  await supabaseApi(token, '/database/query', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ query: sql }),
  })
  console.log('Pricing migration applied.')
}
...
async function applyViaPg() {
  ...
  for (const url of candidates) {
    ...
    try {
      ...
      console.log('Connected. Applying pricing migration…')
...
try {
  ...
  console.error('Paste supabase/add-pricing-units.sql into Supabase SQL Editor.')
  process.exit(1)
}
