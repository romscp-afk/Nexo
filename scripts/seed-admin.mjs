...
const sql = readFileSync(join(root, '..', 'supabase', 'fix-admin-login.sql'), 'utf8')
const candidates = [
  env('SUPABASE_DB_URL'),
...
for (const url of candidates) {
  ...
  try {
    ...
    console.error(`Failed (${url.split('@')[1] ?? url}):`, err.message)
    try {
      await client.end()
    } catch {}
  }
...
console.error('\nCould not connect. Paste supabase/fix-admin-login.sql into the Supabase SQL Editor and run it there.')
process.exit(1)
