#!/usr/bin/env node
/**
 * Apply supabase/add-customer-platform-fee-payment.sql
 * Usage: npm run setup:customer-platform-fee
 */
...
const sql = readFileSync(join(root, '..', 'supabase', 'add-customer-platform-fee-payment.sql'), 'utf8')

async function applyViaManagementApi() {
  const fileEnv = loadEnv()
  const token = requireAccessToken(fileEnv)
  console.log('Applying customer platform fee payment migration via Supabase Management API…')
  await supabaseApi(token, '/database/query', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ query: sql }),
  })
  console.log('Customer platform fee payment migration applied.')
}
...
async function applyViaPg() {
  ...
  for (const url of candidates) {
    ...
    try {
      ...
      console.log('Connected. Applying customer platform fee payment migration…')
...
try {
  ...
  console.error('Paste supabase/add-customer-platform-fee-payment.sql into Supabase SQL Editor.')
  process.exit(1)
}
