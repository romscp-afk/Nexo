...
const benefits = [
  { icon: ShieldCheck, text: 'Cleaning service providers reviewed before activation' },
...
export function AuthLayout() {
  ...
  return (
    <div className={native ? 'flex min-h-dvh flex-col' : 'flex min-h-screen flex-col lg:flex-row'}>
      {!native && (
      ...
      <div className="flex flex-1 flex-col items-center justify-center bg-brand-bg px-4 py-8 pb-[calc(2rem+env(safe-area-inset-bottom))] pt-[calc(2rem+env(safe-area-inset-top))] lg:px-10">
        <div className={native ? 'mb-6 w-full max-w-md' : 'mb-6 w-full max-w-md lg:hidden'}>
        ...
        <p
          className={
            native
              ? 'mt-6 text-center text-sm text-brand-text-muted'
              : 'mt-6 text-center text-sm text-brand-text-muted lg:hidden'
          }
        >
