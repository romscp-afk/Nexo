        ...
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-8">
          ...
          <div className="relative mx-auto h-[340px] w-full max-w-md sm:h-[400px] lg:mx-0 lg:max-w-none">
            <div className="animate-float absolute left-1/2 top-1/2 z-20 w-56 -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-white/20 bg-white p-6 shadow-2xl shadow-black/25 sm:w-64 sm:p-7">
              <div className="mx-auto flex h-28 w-28 items-center justify-center rounded-2xl bg-white sm:h-32 sm:w-32">
                <img src={logoUrl} alt={`${APP_NAME} logo`} className="h-24 w-24 object-contain sm:h-28 sm:w-28" />
              </div>
              <p className="mt-4 text-center text-lg font-bold text-nexo-950">{APP_NAME}</p>
              <p className="mt-1 text-center text-xs text-nexo-800/70">{APP_TAGLINE}</p>
              <div className="mt-4 flex justify-center gap-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
                ))}
              </div>
              <p className="mt-2 text-center text-xs font-medium text-nexo-600">4.9 · 2,400+ bookings</p>
            </div>
